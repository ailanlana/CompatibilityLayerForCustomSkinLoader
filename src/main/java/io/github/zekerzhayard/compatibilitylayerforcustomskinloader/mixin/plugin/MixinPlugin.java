...
import static io.github.zekerzhayard.compatibilitylayerforcustomskinloader.mixin.plugin.TargetedMod.VANILLA;
import static java.nio.file.Files.walk;

public class MixinPlugin implements IMixinConfigPlugin {

    private static final Logger LOG = LogManager.getLogger(CompatibilityLayerForCustomSkinLoader.MODID + " mixins");
    ...
    public List<String> getMixins() {
        ...
        for (TargetedMod mod : TargetedMod.values()) {
            if (loadedMods.contains(mod)) {
                LOG.debug("Found " + mod.modName + "! Integrating now...");
            } else {
                LOG.debug("Could not find " + mod.modName + "! Skipping integration....");
            }
        }
        ...
        for (Mixin mixin : Mixin.values()) {
            if (mixin.shouldLoad(loadedMods)) {
                ...
                LOG.info("CSKIN Loading mixin: " + mixin.mixinClass);
            }
        }
        return mixins;
    }
    ...
    private boolean loadJarOf(final TargetedMod mod) {
        try {
            ...
            LOG.debug("Attempting to add " + jar + " to the URL Class Path");
            if (!jar.exists()) {
